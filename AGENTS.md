...
`APP_VERSION` is automatic: `<major>.<minor>` come from `package.json`, the patch is the commit count on HEAD (`git rev-list --count HEAD`). Every new commit produces a new app version with no manual bump. When git is unavailable (e.g. a Docker build without `.git`), it falls back to the base version in `package.json`.

Current values:

`APP_VERSION: 1.0.<commit count>` — `RENDER_VERSION: 9c84d0752d` — `rv: 9c84d0752d`
