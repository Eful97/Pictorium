...
export function LangPicker({ onPickLang, onPickRegion, onDone }: SetupWizardProps) {
  ...
  const handleSavePin = async (e?: React.FormEvent) => {
    ...
    if (pin.length < 4) {
      setPinError("Il PIN deve contenere almeno 4 cifre")
      return
    }
    setPinLoading(true)
    setPinError(null)
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPinError(err.error || "Errore durante il salvataggio")
      }
    } catch {
      setPinError("Errore di connessione al server")
    } finally {
      setPinLoading(false)
    }
  }
  ...
  const getTitle = () => {
    if (step === "pin") return "Proteggi il tuo pannello"
    if (step === "region") return t("ui.setupRegionTitle")
    return t("ui.welcome")
  }

  const getSubtitle = () => {
    if (step === "pin") return "Imposta un PIN di sicurezza per accedere all'editor e salvare modifiche"
          ...
          <div key="pin" className="surface-card p-6 sm:p-8 rounded-3xl animate-step-enter max-w-sm mx-auto flex flex-col items-center">
            ...
            <form onSubmit={handleSavePin} className="w-full space-y-4">
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-zinc-400 justify-center">
                ...
                <span>Manifest e poster Stremio restano aperti</span>
              </div>
              ...
              >
                <span>{pinLoading ? "Salvataggio..." : "Salva PIN e Inizia"}</span>
